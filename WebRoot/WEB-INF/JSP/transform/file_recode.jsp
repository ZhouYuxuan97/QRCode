	...
	<div class="container-fluid">
									...
									<div class="caption">
										<div class="pull-left z_content" style="padding-left: 50PX; ">
											<h4>
												...
												文件描述：<span>${filelist.detail}</span>
										</div>
										<div class="pull-right">
											<div class="btn-group z_button">
												<button class="btn btn-default" 
													onclick="previewQRCode(${filelist.id})">预览</button>											
												<button class="btn btn-default"
													onclick="editData(${filelist.id})">编辑</button>
												<button class="btn btn-default"
													onclick="deleteData(${filelist.id})">删除</button>
											</div>
										</div>
									</div>
		...
		<p style="text-align: center;">
			<button class="btn btn-default" onclick="AddData()">
				<span class="glyphicon glyphicon-plus"></span> 添加文件码
			</button>
		</p>
		<!--添加-->
	</div>
	<script>
		...
		function AddData() {
			...
			layer.open({
				type : 2,
				area : ['800px','500px'],
				content : "${show}/transformControl/jumpToAddQRCode.action?type=file"
			});
		}
		...
		function editData(id) {
			...
			layer.open({
				type : 2,
				area : ['600px','600px'],
				content : "${show}/transformControl/edit.action?id=" + id + "&type=file"
			});
		}
